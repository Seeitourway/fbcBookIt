...
using viewModels = Web.Areas.SecurityGuard.ViewModels;
using FbcBookIt.Repository;
using FbcBookIt.Entity;
using System.Collections.Generic;

namespace Web.Areas.SecurityGuard.Controllers
{
    ...
    public partial class MembershipController : BaseController
    {
        ...
        private readonly ITeacherR _teacherR;

        public MembershipController(ITeacherR aTeachR)
        {
            this._teacherR = aTeachR as ITeacherR;
        ...
        public virtual ActionResult GrantRolesToUser(string userName, string roles)
        {
            ...
            try
            {
                ...
                foreach(string s in roleNames)
                {
                    ...
                    {
                        // add a teacher object for this user
                        Teacher t = new Teacher();
                        List<MembershipUser> mu = membershipService.FindUsersByName(userName).ToList();
                        t.Email = mu.Find(x => x.UserName == userName).Email;
                        t.Name = mu.Find(x => x.Email == t.Email).UserName;
                        _teacherR.Add(t);
                    }
                }
                response.Success = true;
